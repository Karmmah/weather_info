	...
	for i in range(amount): #draw lines
		...
		for j in range(0,len(forecast)): #draw entries
			if i == 0: #draw vertical separators on first line
				if forecast[j][0][11:13] == "00": #vertical lines; at the start of each day
#					draw.line([(j*width,epd_width),(j*width,epd_width-height*amount)], width=2)
				...
				else: #at each other data point during the day
			...
			draw.line([((j-1)*width,y_left), (j*width,y_right)], width=1) #graph line
			y_left = y_right

#		draw.text((epd_height-xborder_right-23,y0-height*1.1),text=str(min_max[i][1]), font=small_font) #min and max values of each line
#		draw.text((epd_height-xborder_right-23,y0-height*0.58),text=str(min_max[i][0]), font=small_font)
		draw.text((epd_height-xborder_right-21,y0-height*1.13),text=str(min_max[i][1]), font=small_font) #min and max values of each line
		draw.text((epd_height-xborder_right-21,y0-height*0.64),text=str(min_max[i][0]), font=small_font)

	w,h = draw.textsize('W')
	draw.text((epd_height-xborder_right+2,epd_width-height*5.5-h/2),text='T', font=small_font) #labels
	draw.text((epd_height-xborder_right+2,epd_width-height*4.5-h/2),text='W', font=small_font)
	draw.text((epd_height-xborder_right+2,epd_width-height*3.5-h/2),text='C', font=small_font)
	draw.text((epd_height-xborder_right+2,epd_width-height*2.5-h/2),text='R', font=small_font)
	draw.text((epd_height-xborder_right+2,epd_width-height*1.5-h/2),text='P', font=small_font)
	draw.text((epd_height-xborder_right+2,epd_width-height*0.5-h/2),text='H', font=small_font)
